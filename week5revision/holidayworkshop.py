...
def valid_workshop_code(workshop_code):
    ...
    if len(workshop_code) != 3:
        print("Your workshop code does not contain exactly three characters. ")
        invalid_counter += 1
        return False
    if workshop_code.upper() not in ["ROB", "WEB", "PYT"]:
        print("Your workshop code is not ROb, WEB or PYT. ")
        invalid_counter += 1
        return False
...
def calculate_booking_fee(workshop_code, number_of_students):
    if workshop_code == "ROB":
        total_fee = 48 * number_of_students
    elif workshop_code == "WEB":
        total_fee = 36 * number_of_students
    else:
        total_fee = 42 * number_of_students
