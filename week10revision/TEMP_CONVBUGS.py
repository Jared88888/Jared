...
def getConvertTo():
    which = input("Enter selection: ") #1 add quotation and bracket
    while which != "F" and which != "C": #3 capital C, #4, change to and, !=
        which = input("Enter selection: ") #2 add indent
    return which
...
which = getConvertTo() #5, change to which

# Get range of temperatures to convert
temp_start = int(input("Enter starting temperature to convert: "))
temp_end = int(input("Enter ending temperature to convert: ")) #6, int
