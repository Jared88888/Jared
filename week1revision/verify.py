...
for i in range(5):
    ...
    if ID[0] == "S" or ID[0] == "T":
        print("Welcome home!")
        singaporeans.append(ID)
    else:
        print("Welcome to Singapore!")
print(f"The number of Singaporeans is {len(singaporeans)}. ")
...
while True:
    ...
    if len(ID) != 9:
        while True:
            print("Error, length of ID must be 9 characters. ")
            ID = input("Enter ID. ")
            if len(ID) == 9:
                break
    if ID[0] == "S" or ID[0] == "T":
        ...
        print("Welcome to Singapore!")
    more = input("One more entry? (Y/N)").upper()
    if more == "N":
        break
print(f"The number of Singaporeans is {len(singaporeans)}. ")